        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          <div>
            ...
            <div className="flex gap-3">
              ...
              <a href="https://x.com/pawsandtreks" className="w-9 h-9 bg-background/10 rounded-md flex items-center justify-center hover-elevate transition-colors" aria-label="X (Twitter)" data-testid="link-social-x" target="_blank" rel="noopener noreferrer">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              </a>
              <a href="https://www.youtube.com/@pawsandtreks" className="w-9 h-9 bg-background/10 rounded-md flex items-center justify-center hover-elevate transition-colors" aria-label="YouTube" data-testid="link-social-youtube" target="_blank" rel="noopener noreferrer">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {["Maasai Mara", "Amboseli", "Tsavo", "Lake Nakuru", "Samburu", "Nairobi National Park"].map((dest) => (
                <li key={dest}>
                  <Link href="/#destinations" className="text-background/60 text-sm transition-colors" data-testid={`link-footer-dest-${dest.toLowerCase().replace(/\s/g, "-")}`}>
                    {dest}
                  </Link>
                </li>
              ))}
